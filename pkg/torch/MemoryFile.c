...
{
  long missingSpace;
  long storageSize = THCharStorage_size(file->storage);
  if(size <= file->size)
    return;
  else
  {
    if(size < storageSize) /* note the "<" and not "<=" */
    {
      file->size = size;
      THCharStorage_set(file->storage, file->size, '\0');
      return;
    }
  }

  missingSpace = size-storageSize+1; /* +1 for the '\0' */
  THCharStorage_resize(file->storage, (storageSize/2 > missingSpace ?
                                       storageSize + (storageSize/2)
                                       : storageSize + missingSpace));
}
...
\
  ...
  { \
    ...
    { \
      ...
      array = TH##TYPEC##Storage_data(storage); \
    } \
    else if( (storage = luaT_toudata(L, 2, torch_##TYPEC##Storage_id)) ) \
    { \
      n = TH##TYPEC##Storage_size(storage); \
      array = TH##TYPEC##Storage_data(storage); \
    } \
    else \
...
\
  ...
  { \
    ...
    memmove(array, THCharStorage_data(file->storage)+file->position, result*sizeof(TYPE)); \
  ...
  { \
    ...
    { \
      long nByteRead = 0; \
      char spaceChar = 0; \
      char *spacePtr = torch_MemoryFile_c_strnextspace(THCharStorage_data(file->storage)+file->position, &spaceChar); \
      ASCII_READ_ELEM; \
      if(ret == EOF) \
      { \
        while(THCharStorage_get(file->storage, file->position)) \
          file->position++; \
      } \
      else \
    ...
    { \
      if( (file->position < file->size) && (THCharStorage_get(file->storage, file->position) == '\n') ) \
        file->position++; \
    } \
  } \
\
...
\
  ...
  { \
    ...
    memmove(THCharStorage_data(file->storage)+file->position, array, nByte); \
    file->position += nByte; \
  ...
  { \
    ...
    { \
      ...
      { \
        ASCII_WRITE_ELEM; \
        if( (nByteWritten > -1) && (nByteWritten < THCharStorage_size(file->storage)-file->position) ) \
        { \
          file->position += nByteWritten; \
          break; \
        } \
        torch_MemoryFile_c_grow(file, THCharStorage_size(file->storage) + (THCharStorage_size(file->storage)/2) + 2); \
      } \
      ...
      { \
        ...
        { \
          ...
          sprintf(THCharStorage_data(file->storage)+file->position, " "); \
          file->position++; \
        } \
        if(i == n-1) \
        { \
    ...
    { \
      file->size = file->position; \
      THCharStorage_data(file->storage)[file->size] = '\0'; \
    } \
  } \
\
  lua_pushnumber(L, n); \
...
static int torch_MemoryFile_new(lua_State *L)
{
  ...
  if(storage)
  {
    luaL_argcheck(L, THCharStorage_get(storage, THCharStorage_size(storage)-1) == '\0', 1, "provided CharStorage must be terminated by 0");
  ...
  else
  {
    ...
    THCharStorage_set(storage, 0, '\0');
  }

  file = luaT_alloc(L, sizeof(MemoryFile));
  file->storage = storage;
  file->size = (storage ? THCharStorage_size(storage)-1 : 0);
  file->position = 0;
...
static int torch_MemoryFile_storage(lua_State *L)
{
  ...
  THCharStorage_retain(file->storage);
...
/*                    int value = 0; int ret = sscanf(THCharStorage_data(file->storage)+file->position, "%d%n", &value, &nByteRead); array[i] = (value ? 1 : 0), */
/*                    int value = (array[i] ? 1 : 0); nByteWritten = snprintf(THCharStorage_data(file->storage)+file->position, THCharStorage_size(file->storage)-file->position, "%d", value), */
/*                    1) */
...
READ_WRITE_METHODS(char, Char,
                   ...
                   result = ret; \
                   i = n-1; \
                   memmove(array, THCharStorage_data(file->storage)+file->position, nByteRead),
                   nByteWritten = (n < THCharStorage_size(file->storage)-file->position ? n : -1); \
                   i = n-1; \
                   if(nByteWritten > -1)
                     memmove(THCharStorage_data(file->storage)+file->position, array, nByteWritten),
                   0)

READ_WRITE_METHODS(short, Short,
                   int nByteRead_; int ret = sscanf(THCharStorage_data(file->storage)+file->position, "%hd%n", &array[i], &nByteRead_); nByteRead = nByteRead_; if(ret <= 0) break; else result++,
                   nByteWritten = snprintf(THCharStorage_data(file->storage)+file->position, THCharStorage_size(file->storage)-file->position, "%hd", array[i]),
                   1)

READ_WRITE_METHODS(int, Int,
                   int nByteRead_; int ret = sscanf(THCharStorage_data(file->storage)+file->position, "%d%n", &array[i], &nByteRead_); nByteRead = nByteRead_; if(ret <= 0) break; else result++,
                   nByteWritten = snprintf(THCharStorage_data(file->storage)+file->position, THCharStorage_size(file->storage)-file->position, "%d", array[i]),
                   1)

READ_WRITE_METHODS(long, Long,
                   int nByteRead_; int ret = sscanf(THCharStorage_data(file->storage)+file->position, "%ld%n", &array[i], &nByteRead_); nByteRead = nByteRead_; if(ret <= 0) break; else result++,
                   nByteWritten = snprintf(THCharStorage_data(file->storage)+file->position, THCharStorage_size(file->storage)-file->position, "%ld", array[i]),
                   1)

READ_WRITE_METHODS(float, Float,
                   int nByteRead_; int ret = sscanf(THCharStorage_data(file->storage)+file->position, "%g%n", &array[i], &nByteRead_); nByteRead = nByteRead_; if(ret <= 0) break; else result++,
                   nByteWritten = snprintf(THCharStorage_data(file->storage)+file->position, THCharStorage_size(file->storage)-file->position, "%g", array[i]),
                   1)

READ_WRITE_METHODS(double, Double,
                   int nByteRead_; int ret = sscanf(THCharStorage_data(file->storage)+file->position, "%lg%n", &array[i], &nByteRead_); nByteRead = nByteRead_; if(ret <= 0) break; else result++,
                   nByteWritten = snprintf(THCharStorage_data(file->storage)+file->position, THCharStorage_size(file->storage)-file->position, "%g", array[i]),
                   1)
...
static int torch_MemoryFile_readString(lua_State *L)
{
  ...
  if(format[1] == 'a')
  {
    lua_pushlstring(L, THCharStorage_data(file->storage)+file->position, file->size-file->position);
    file->position = file->size;
    return 1;
  }
  else
  {
    char *p = THCharStorage_data(file->storage)+file->position;
    long posEol = -1;
    long i;
    ...
    if(posEol >= 0)
    {
      lua_pushlstring(L, THCharStorage_data(file->storage)+file->position, posEol); /* do not copy the end of line */
...
static int torch_MemoryFile_writeString(lua_State *L)
{
  ...
  memmove(THCharStorage_data(file->storage)+file->position, str, size);
  file->position += size;
  if(file->position > file->size)
  {
    file->size = file->position;
